...
object ProotBootstrap {
  ...
  private fun download(urlStr: String, dst: File, log: (String) -> Unit) {
    ...
    BufferedInputStream(conn.inputStream).use { i ->
  ...
  private fun runBsdtar(bsdtar: File, tarball: File, rootfs: File, log: (String) -> Unit): Boolean {
    return try {
      ...
      log("bsdtar error: ${e.message}")
      false
    }
  }

  private fun patchRootfs(rootfs: File, log: (String) -> Unit) {
    // Placeholder for rootfs patching logic (e.g. fixing symlinks or permissions)
    log("Patching rootfs...")
  }
}
